...
fn build_lib(src_path: &Path) {
    ...
    build_cmd.arg("--target").arg("target.json");
    build_cmd.arg("-Zbuild-std=core,alloc");
    build_cmd.arg("-Zbuild-std-features=compiler-builtins-mem");
    build_cmd.arg("-Zunstable-options");
    ...
    ar.arg("crus").arg(lib_path).arg(obj_path);
