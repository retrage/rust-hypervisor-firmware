...
use crate::block::{Capacity, Error as BlockError, SectorRead, SectorWrite};
...
impl<'a> Device<'a> {
    pub fn new(transport: &'a mut dyn VirtioTransport) -> Device<'a> {
        Device {
            transport,
    ...
    fn request(
        ...
        match footer.status {
            VIRTIO_BLK_S_OK => Ok(()),
            VIRTIO_BLK_S_IOERR => Err(BlockError::BlockIOError),
