...
impl Transport {
    pub fn new(device: pci::PciDevice) -> Transport {
        Transport {
            device,
            ..Default::default()
        }
    ...
    fn init(&mut self, _device_type: u32) -> Result<(), VirtioError> {
        ...
        while cap_next < 0xff && cap_next > 0 {
            ...
            if self.device.read_u8(cap_next) == 0x09 {
                ...
                if cfg_type == CapabilityType::CommonConfig as u8 {
                    self.region = mem::MemoryRegion::new(
                        self.device.get_bar(usize::from(bar)) + u64::from(offset),
                        u64::from(length),
                    );
                }

                if cfg_type == CapabilityType::NotifyConfig as u8 {
                ...
                if cfg_type == CapabilityType::DeviceConfig as u8 {
